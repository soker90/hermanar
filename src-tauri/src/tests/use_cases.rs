...
fn test_reporte_cuotas_anio() {
    ...
    // Total cuotas 2025
    let cuotas_2025: i32 = conn
        .query_row("SELECT COUNT(*) FROM cuotas WHERE anio = 2025", [], |row| {
            row.get(0)
        })
        .unwrap();

    // Total cobrado 2025
    let cobrado_2025: f64 = conn
        .query_row(

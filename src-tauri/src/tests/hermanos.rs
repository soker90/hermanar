...
fn test_create_hermano_with_familia() {
    ...
    let conn = db.lock().unwrap();
    let stored_familia_id: i32 = conn.query_row(
...
fn test_search_hermano_by_nombre() {
    ...
    let mut stmt = conn.prepare(
        "SELECT COUNT(*) FROM hermanos WHERE nombre LIKE ?1"
    ).unwrap();
    
    let count: i32 = stmt.query_row(
        rusqlite::params!["%Juan%"],
        |row| row.get(0),
    ).unwrap();
...
fn test_search_hermano_by_apellido() {
    ...
    let conn = db.lock().unwrap();
    let mut stmt = conn.prepare(
        "SELECT COUNT(*) FROM hermanos WHERE primer_apellido LIKE ?1"
    ).unwrap();
...
fn test_update_hermano_familia() {
    ...
    let hermano_id = insert_test_hermano(&db, "00001", Some("Juan"), Some("García"), Some(f1)).unwrap();
    ...
    let conn = db.lock().unwrap();
    let familia_id: i32 = conn.query_row(
        "SELECT familia_id FROM hermanos WHERE id = ?1",
        rusqlite::params![hermano_id],
        |row| row.get(0),
    ).unwrap();
...
fn test_hermanos_por_familia_count() {
    ...
    let conn = db.lock().unwrap();
    let f1_count: i32 = conn.query_row(
        "SELECT COUNT(*) FROM hermanos WHERE familia_id = ?1",
        rusqlite::params![f1],
        |row| row.get(0),
    ).unwrap();
    
    let f2_count: i32 = conn.query_row(
        "SELECT COUNT(*) FROM hermanos WHERE familia_id = ?1",
        rusqlite::params![f2],
        |row| row.get(0),
    ).unwrap();
...
fn test_hermano_inactivo_no_aparece_en_activos() {
    ...
    let conn = db.lock().unwrap();
    let activos: i32 = conn.query_row(
        "SELECT COUNT(*) FROM hermanos WHERE activo = 1",
        [],
        |row| row.get(0),
    ).unwrap();
    
    let inactivos: i32 = conn.query_row(
...
fn test_cmd_restriccion_hermano_numero_unico() {
    ...
    assert!(result.is_err(), "Comando debería rechazar número de hermano duplicado");
}

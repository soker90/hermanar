...
pub fn borrar_base_datos_cmd(_db: State<DbConnection>) -> Result<String, String> {
    let db_path = get_database_path().map_err(|e| format!("Error al obtener ruta de BD: {}", e))?;
    ...
    let now = Local::now();
    let filename = format!(
        "hermanar-backup-antes-borrar-{}",
        now.format("%Y-%m-%d-%H-%M-%S")
    );

    // Leer el archivo de base de datos
    let db_data =
        fs::read(&db_path).map_err(|e| format!("Error al leer la base de datos: {}", e))?;
    ...
    fs::remove_file(&db_path).map_err(|e| format!("Error al borrar la base de datos: {}", e))?;

    Ok(format!(
        "Backup creado en: {}. Base de datos borrada exitosamente. La aplicación se reiniciará.",
...
pub fn exportar_backup_cmd(_db: State<DbConnection>) -> Result<String, String> {
    ...
    // Comprimir usando zstd
    let compressed_data =
        zstd::encode_all(&db_data[..], 3).map_err(|e| format!("Error al comprimir: {}", e))?;
...
pub fn importar_backup_cmd(db: State<DbConnection>, backup_path: String) -> Result<String, String> {
    ...
    let compressed_data =
        fs::read(&backup_file).map_err(|e| format!("Error al leer archivo de backup: {}", e))?;
    ...
    let validation_result = Connection::open(&temp_db_path).and_then(|conn| {
    ...
    validation_result
        .map_err(|e| format!("La base de datos está corrupta o no es válida: {}", e))?;
